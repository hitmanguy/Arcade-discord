...
export default new SlashCommand({
  ...
  data: commandData as SlashCommandBuilder,
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    try {
      ...
      console.error('Error in numbers game command:', error);
...
async function handleCreateGame(interaction: ChatInputCommandInteraction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Game Rules', value: 'Each round, players choose a number between 1-100. The closest unique number to the average wins!\nPhase 1 (5-4 players): Standard rounds\nPhase 2 (3 players): Betrayal phase\nPhase 3 (2 players): Final duel' }
    )
    .setColor(0x00ff00)
...
async function handleStartGame(interaction: ChatInputCommandInteraction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Phase 1', value: 'Standard rounds - choose numbers, closest unique to average wins!' }
    )
    .setColor(0x0000ff);
  ...
  try {
    ...
    while (gameState.getAlivePlayers().length > 1) {
      ...
      if (alivePlayers.length >= 4) {
        ...
        await channel.send('### 🔄 Entering Betrayal Phase!\nWith only 3 players remaining, you can now form alliances... or betray them!');
        await gameState.runBetrayalPhase(channel);
      } else if (alivePlayers.length === 2) {
        // Phase 3: Final duel
        await channel.send('### ⚔️ Final Duel Phase!\nThe last two players face off in a battle of wits!');
    ...
    await channel.send('An error occurred during the game. The game has been terminated.');
  } finally {
    // Clean up the game
    gameState.endGame();
  }
